...
class SubsetDiversitySampler(Sampler):
    ...
    def __init__(self, valid_indices : list, diversity_path : str, diversity_cutoff : float, max_size: int, strategy : str = "maximize", seed : int = 123) -> None:
        """
        Args:
            valid_indices (list): list of valid dataset indices
        ...
        print(f"=== USING {len(self.indices)} out of {len(self.valid_indices)} samples ===")
    def subset_by_cutoff(self, cutoff, cutoff_func) -> None:
        for row in self.diversity_data.itertuples():
            ...
            if is_cutoff or len(self.indices) >= self.max_size:
                break
            elif row.Index in self.valid_indices:
